...
test "optimizer sgd with momentum" {
    ...
    try expectNear(lyr.weights[0], 0.499, 0.0001);

    lyr.grad_weights[0] = 0.2;
    sgd.step(lyr, 0.01);

    try expectNear(lyr.weights[0], 0.4961, 0.001);
...
test "optimizer adam with known values" {
    ...
    try testing.expect(lyr.weights[0] < 0.5);
}
...
test "optimizer rmsprop basic" {
    ...
    var rmsprop: optimizer.Rmsprop = .{
        .g_weights = try allocator.alloc(f32, lyr.weights.len),
        .g_bias = try allocator.alloc(f32, lyr.bias.len),
    };
...
test "optimizer sgd vs no momentum" {
    ...
    lyr1.weights[0] = 1.0;
    lyr1.grad_weights[0] = 0.1;

    sgd_no_mom.step(lyr1, 0.1);
    const w1_after = lyr1.weights[0];
    ...
    lyr2.weights[0] = 1.0;
    lyr2.grad_weights[0] = 0.1;

    sgd_mom.step(lyr2, 0.1);
    const w2_after = lyr2.weights[0];
...
test "optimizer: multiple layers with different sizes" {
    ...
    var lyr2 = try layer.Dense.init(allocator, 3, 1, .sigmoid, backend.Backend{ .cpu = {} });
    defer lyr2.deinit();
