...
test "optimizer sgd: momentum weight update" {
    ...
    try testing.expectNear(lyr.weights[0], 0.9961, 0.0001);
}
...
test "optimizer sgd: momentum bias update" {
    ...
    try testing.expectNear(lyr.bias[0], 0.499, 0.0001);
    try testing.expectNear(lyr.bias[1], 0.998, 0.0001);
    ...
    try testing.expectNear(lyr.bias[0], 0.4961, 0.001);
    try testing.expectNear(lyr.bias[1], 0.9932, 0.001);
}
...
test "optimizer sgd: no momentum" {
    ...
    try testing.expectNear(lyr.weights[0], 0.99, 0.0001);
    try testing.expectNear(lyr.weights[1], 1.98, 0.0001);
}
...
test "optimizer sgd: multiple weight updates" {
    ...
    const allocator = testing.allocator;
    var lyr = try layer.Dense.init(allocator, 3, 2, .relu, backend.Backend{ .cpu = {} });
    defer lyr.deinit();
...
test "optimizer sgd: comparison with manual calculation" {
    ...
    lyr.weights[0] = 5.0;
    lyr.grad_weights[0] = 0.5;

    // Step 1: v = 0.8 * 0 + 0.5 = 0.5, w = 5.0 - 0.01 * 0.5 = 4.995
    sgd.step(&lyr, 0.01);
    try testing.expectNear(lyr.weights[0], 4.995, 0.0001);

    // Step 2: v = 0.8 * 0.5 + 0.5 = 0.9, w = 4.995 - 0.01 * 0.9 = 4.986
    lyr.grad_weights[0] = 0.5;
    sgd.step(&lyr, 0.01);
    try testing.expectNear(lyr.weights[0], 4.986, 0.0001);
}
...
test "optimizer sgd: momentum prevents oscillation" {
    ...
    // With momentum
    var lyr1 = try layer.Dense.init(allocator, 1, 1, .relu, backend.Backend{ .cpu = {} });
    defer lyr1.deinit();
    try sgd_with_momentum.init(allocator, &lyr1);

    // Without momentum
    var lyr2 = try layer.Dense.init(allocator, 1, 1, .relu, backend.Backend{ .cpu = {} });
    defer lyr2.deinit();
    ...
    lyr1.grad_weights[0] = 1.0;
    lyr2.grad_weights[0] = 1.0;
    sgd_with_momentum.step(&lyr1, 0.1);
    sgd_no_momentum.step(&lyr2, 0.1);

    const w1_after_first = lyr1.weights[0];
    const w2_after_first = lyr2.weights[0];
    ...
    _ = std.math.abs(lyr1.weights[0] - w1_after_first);
    _ = std.math.abs(lyr2.weights[0] - w2_after_first);

    // Both should be valid, but momentum should reduce oscillation
    try testing.expect(std.math.isFinite(lyr1.weights[0]));
    try testing.expect(std.math.isFinite(lyr2.weights[0]));
}
...
test "optimizer sgd: convergence with noisy gradients" {
    ...
    for (0..50) |i| {
        ...
        lyr.grad_weights[0] = 0.1 + noise;
        sgd.step(&lyr, 0.01);
    }

    try testing.expect(std.math.isFinite(lyr.weights[0]));
    try testing.expect(lyr.weights[0] < 1.0);  // Should have decreased overall
}
...
test "optimizer sgd: multiple layers" {
    ...
    var lyr1 = try layer.Dense.init(allocator, 2, 3, .relu, backend.Backend{ .cpu = {} });
    defer lyr1.deinit();

    var lyr2 = try layer.Dense.init(allocator, 3, 1, .relu, backend.Backend{ .cpu = {} });
    defer lyr2.deinit();
...
test "optimizer sgd: step interface with different sizes" {
    ...
    // Large layer
    var lyr2 = try layer.Dense.init(allocator, 10, 10, .relu, backend.Backend{ .cpu = {} });
    defer lyr2.deinit();
