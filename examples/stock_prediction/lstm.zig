...
pub fn main() !void {
    ...
    // 1. Generate synthetic stock data (sine wave with noise)
    const data_len = 200;
    ...
    // backend.type = .cpu; // Force CPU for this example if needed
    var net = try zn.network.Network.init(allocator, backend);
    defer net.deinit();

    // Input: window_size, Output: 1
    _ = try net.addLSTM(1, 16, window_size);
    _ = try net.addDense(16, 1, .linear);

    // 5. Train
    std.debug.print("Starting training LSTM stock predictor...\n", .{});
    const epochs = 50;
